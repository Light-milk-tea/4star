...
const basicDPS = computed(() => {
  const { attack, attackSpeed } = operator.value
  return (attack * attackSpeed) / 1.0
})

const skillDPS = computed(() => {
  const { attack, attackSpeed, damageScale, hitCount } = operator.value
  return (attack * attackSpeed * damageScale * hitCount) / 1.0
})

const totalDamage = computed(() => {
  return skillDPS.value * 10 // 假设10秒输出
})

const attackInterval = computed(() => {
  return 1.0 / operator.value.attackSpeed
})
...
function loadPreset(preset: OperatorPreset) {
  operator.value = {
    ...
    damageScale: preset.damageScale,
    hitCount: preset.hitCount
  }
}
</script>
...
.calculator-hero {
  background: linear-gradient(135deg, var(--accent-600) 0%, var(--purple-600) 100%);
  color: var(--white);
...
.result-card {
  background: linear-gradient(135deg, var(--accent-50) 0%, var(--purple-50) 100%);
